...
public class CrunchifyInMemoryCache<K, T> {
  ...
  public CrunchifyInMemoryCache(long crunchifyTimeToLive, final long crunchifyTimerInteval, int maxItems) {
    this.timeToLive = crunchifyTimeToLive * 1000;
    crunchifyCacheMap = new LRUMap(maxItems);
    if (timeToLive > 0 && crunchifyTimerInteval > 0) {
      Thread t = new Thread(new Runnable() {
        @Override
        public void run() {
          while (true) {
            try {
              //Thread: A thread iss a thread of execution in a program
              // The JVM allows an application to have multiple threads of excution running concurrently.
              Thread.sleep(crunchifyTimerInteval * 1000);
      ...
      // start(): Causes this thread to begin execution; the JVM calls the run method of this thread.
  ...
  public void crunchifyCleanup() {
    ...
    synchronized (crunchifyCacheMap) {
      MapIterator iterator = crunchifyCacheMap.mapIterator();
      ...
      while (iterator.hasNext()) {
        key = (K) iterator.next();
        c = (CrunchifyCacheObject) iterator.getValue();
